...
| Live execution | 76 | Functional MV3 extension: scrapes the table, acts, never-freeze fallback. Fragile to PokerNow DOM changes; that's the cap. |
...
|---|---|---:|---:|---|---|
| v0.1.43 | 2026-06-27 | 479 | **68** | + real bet-sizing variety (size head was degenerate) | **Autonomous-loop pass.** Found the learned size head was degenerate — it returned ~0.66 pot for EVERY spot, so the bot bet a flat 65% everywhere (a sizing tell). Replaced with texture+street+strength sizing: dry 60% / monotone 55% / wet 75% / very-wet 85%, river polarized (value 90%, thin pair 50%, bluff 80%). Also validated the whole bot via a 169-hand behavioral audit — all core frequencies (3bet/4bet defense, c-bet 57%, turn 47%, river 40%) in GTO range. Confirmed the net is at its data ceiling (768×384 retrain: TEST 84.95% vs 84.5% = noise + overfitting) and its facing-bet errors are caught by the soundness gate — not a lever. Sim (1500h) GTO 170 / EXPLOIT 171 bb/100. |
| v0.1.42 | 2026-06-27 | 479 | **68** | + executor plays the DECIDED action (was re-rolling) | **Critical execution fix — the bot now does what it decides.** Bot-vs-bot hands exposed three things all rooted in the *executor*, not the brain: (1) it **re-sampled a fresh random action** from the distribution instead of playing `decision.action`, so the move could differ from the overlay's recommendation and looked "random" (e.g. ~25% each preflop); (2) it **min-bet** (20 into a 1000 pot with trip queens) when the React sizing box silently kept the table minimum. Now the executor plays the exact decided action; the raise flow **reads back the amount** and uses a preset if the input didn't register, and **bails to a safe line rather than ever submitting a min-bet**. Overlay recommendation == executed action. |
